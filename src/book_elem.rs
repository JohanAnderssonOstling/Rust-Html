...
use floem::cosmic_text::{Attrs, AttrsList, TextLayout};
    ...
    pub fn add_child (&mut self, elem: Elem) {
        match &elem.elem_type {
            ElemType::Block(block) => {
                self.total_child_count += block.total_child_count;
            }
...
pub struct ElemLines {
    height: f64,
    pub elem_lines: Vec<ElemLine>
}
#[derive(Clone)]

pub struct ElemLine {
    pub height: f64,
    pub inline_elems: Vec<InlineElem>
}
#[derive(Clone)]
...
impl BookElemFactory {
    pub fn new() -> Self{
        BookElemFactory {curr_x: 0., curr_y: 0.}
    }
}
...
impl BookElemFactory {
    ...
    pub fn parse(&mut self, node: Node) -> Elem {
        if node.tag_name().name().eq("html") {
            for child in node.children() {
                if child.tag_name().name().eq("body") { return self.parse(child); }
            }
        }
        let mut block_elem = BlockElem {children: Vec::new(), total_child_count: 0};
        let mut inline_items: Vec<InlineItem> = Vec::new();
        let font = Attrs::new().font_size(20.);
        let attrs_list = AttrsList::new(Attrs::new().font_size(20.));
        let init_point = Point::new(self.curr_x, self.curr_y);
        let mut top = 0.;
        let mut bottom = 0.;
        ...
        for child in node.children() {
            ...
            if BLOCK_ELEMENTS.contains(&tag_name) {
                ...
                block_elem.add_child(self.parse(child));
            }
    ...
    pub fn parse_inline(&self, node: Node, attrs_list: AttrsList) -> Vec<InlineItem> {
        ...
        for child in node.children() {
            if child.tag_name().name().eq("") {
                inline_items.extend(self.parse_text(child, attrs_list.clone()));
            }
            else {
                inline_items.extend(self.parse_inline(child, attrs_list.clone()));
            }
        }
        inline_items
    }
    ...
    pub fn parse_text(&self, node: Node, attrs_list: AttrsList) -> Vec<InlineItem> {
       ...
       for word in node.text().unwrap().split(" ") {
            ...
            let inline_content = InlineContent::Text(text_layout);
